...
import datetime
from circuits_minpor.portal import portal_update, portal_message, PortalView
...
class ServerTimePortlet(TemplatePortlet):
    ...
    def _update_time(self, session):
        ...
        td = int(td)
        self.fire(portal_update(self, session, "new_time", \
                                str(td)), self._portal_channel)
    ...
    def _on_off_changed(self, event, session, value, **kwargs):
        if value and self._timer is None:
            ...
            self.fire(portal_message(session, self.translation(locales) \
